...
DoAcid = function(time)
  ...
  local sans = math.random(1,3)
  print(sans)
  if sans == 1 then
    print("1. şarkı aktif")
    SendNUIMessage({type = "playMusic", song = song})
  elseif sans == 2 then
    SendNUIMessage({type = "playMusiciki", song = song})
    print("2. şarkı aktif")
  elseif sans == 3 then
    SendNUIMessage({type = "playMusicuc", song = song})
    print("3. şarkı aktif")
  end
  ...
  QBCore.Functions.Notify("Rolünü bölmemek için yürüme animasyonunu değiştirmedim, F3 Animasyonlardan değiştirebilirsin. İyi Roller!", "error")
...
RegisterNetEvent('aiakos:acidtrip')
AddEventHandler('aiakos:acidtrip', function(source)
  local suresansi = math.random(50000,150000)
  if evet == false then
    print("Süre" .. suresansi .. "")
    evet = true
    QBCore.Functions.Notify("Dikkat Epilepsi uyarısı eğer epilepsi hastasıysanız bir şekilde ekranı kapatın ve 5 dakika sonra tekrar oyuna geliniz!", "error")
    ClearPedTasksImmediately(PlayerPedId())
    TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_DRUG_DEALER", 0, true)
    QBCore.Functions.Progressbar("sigaraacid", "Tüttürüyorsun", 15000*3, false, true, { -- p1: menu name, p2: yazı, p3: ölü iken kullan, p4:iptal edilebilir
      ...
      QBCore.Functions.Notify("Yeterince içtin!", "error")
  end
end)
